...
int main(void)
{
	...
	while (arts[++len]);
	struct s_dict *dict = dictInit(len);
	while (++i < len){
		dictInsert(dict, arts[i]->name, arts[i]);
	}
	printf("Price for the art piece \'%s\' is %i\n", "Guernica", searchPrice(dict, "Guernica"));
	printf("Price for the art piece \'%s\' is %i\n", "THE MYSTERY OF THE SEVEN STARS (plate, folio 7 verso) from THE APOCALYPSE", searchPrice(dict, "THE MYSTERY OF THE SEVEN STARS (plate, folio 7 verso) from THE APOCALYPSE"));
	printf("Price for the art piece \'%s\' is %i\n", "me in the shower", searchPrice(dict, "me in the shower"));
	printf("Price for the art piece \'%s\' is %i\n", "Mona Lisa", searchPrice(dict, "Mona Lisa"));
	printf("Price for the art piece \'%s\' is %i\n", "I don't exist!", searchPrice(dict, "I don't exist!"));
	return (0);
}
